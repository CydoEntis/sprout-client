...
function CreateTaskListModal({ isOpened, onClose }: CreateTaskListModalProps) {
  ...
  const { mutateAsync: createTaskList, isPending } = useCreateTaskListMutation(categoryName);
    ...
    <Modal opened={isOpened} onClose={onClose} title="Create a new list">
      <form onSubmit={form.onSubmit(handleSubmit)}>
        <Stack gap={16}>
          ...
          <Button type="submit" w="100%" variant="light" color="lime" disabled={isPending} loading={isPending}>
            Create List
          </Button>
        </Stack>
      </form>
